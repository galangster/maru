  ...
  return (
    <BottomSheet title={count > 1 ? `Save ${count} threads for later` : 'Save for later'} onClose={onClose}>
      <div className="mobile-later-options">
        {presets.map((preset) => <button type="button" key={preset.id} onClick={() => onPick(preset.wakeAt)}><span className="mobile-sheet-icon"><Clock3 size={19} /></span><span><strong>{preset.label}</strong><small>{preset.detail}</small></span><ChevronRight size={17} /></button>)}
        <label className="mobile-custom-date"><span className="mobile-sheet-icon"><Clock3 size={19} /></span><span><strong>Pick a date</strong><small>Up to {MAX_DEFER_DAYS} days</small></span><input type="date" aria-label="Bring it back on" min={isoDay(now + 86_400_000)} max={isoDay(maxDeferAt(now))} onChange={(event) => { const at = clampedDeferDay(event.target.value, now); if (at !== null) onPick(at) }} /></label>
      </div>
