  ...
  return (
    <section className="mobile-screen mobile-settings" aria-label="Settings">
      ...
      <div className="mobile-scroll mobile-settings-scroll">
        <SettingsGroup title="Accounts">{accounts.map((account) => <SettingsRow key={account.id} icon={<MobileIcon name="participants" scale="action" />} title={account.displayName} detail={account.email} />)}</SettingsGroup>
        ...
        <SettingsGroup title="About"><SettingsRow icon={<MobileIcon name="info" scale="action" />} title="Maru for iPhone" detail="Version 0.1.7 · Demo mode" /></SettingsGroup>
      </div>
    </section>
  )
}
