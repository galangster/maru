...
import { Image as ImageIcon, Info, Mail, UserRound, Volume2 } from 'lucide-react'
  ...
  return (
    <section className="mobile-screen mobile-settings" aria-label="Settings">
      ...
      <div className="mobile-scroll mobile-settings-scroll">
        ...
        <SettingsGroup title="Maru account"><SettingsRow icon={<Mail size={19} />} title="Maru account" detail="Coming with sync" /></SettingsGroup>
...
function SettingsRow({ icon, title, detail }: { icon: ReactNode; title: string; detail: string }) {
  return <div className="mobile-settings-row"><span className="mobile-settings-icon">{icon}</span><span><strong>{title}</strong><small>{detail}</small></span></div>
}
